...
void CXMPPServer::connectToServer()
{
	...
	connect(logger, SIGNAL(message(QXmppLogger::MessageType, QString)), this,
	        SLOT(qxmppLogEvent(QXmppLogger::MessageType, QString)));
...
{
	if (botInstance->config()["servers"].toMap().value(m_handle).toMap().value("users").toMap().value(username).toString() ==
	    password)
		return true;
	...
	if (args[0] == "autorun") {
		...
		if (room) {
			...
			if (script) {
				...
				if (script->autoRun())
					m_client->sendMessage(msg.from(), fmt("AUTORUN flag on script '%1' "
					                                      "in room '%2' is now ON")
					                                      .arg(script->script())
					                                      .arg(room->roomName()));
				else
					m_client->sendMessage(msg.from(), fmt("AUTORUN flag on script '%1' "
					                                      "in room '%2' is now OFF")
					                                      .arg(script->script())
					                                      .arg(room->roomName()));
			}
		}
