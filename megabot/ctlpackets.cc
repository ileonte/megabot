...
{
	if ( data.size() - p < 4 ) return false;
	qint32 l = *( ( qint32 * )( data.data() + p ) ); p += 4;
	if ( p + l > data.size() ) return false;
...
bool CRoomConfigPacket::unpack( QByteArray &from )
{
	...
	if ( !deconstructPacket( from, data ) ) return false;
...
bool CRoomMessagePacket::unpack( QByteArray &from )
{
	...
	if ( !deconstructPacket( from, data ) ) return false;
	if ( !unpack_string( data, p, m_from ) ) return false;
	if ( !unpack_string( data, p, m_body ) ) return false;
	if ( !unpack_int( data, p, m_serverTime ) ) return false;
	if ( !unpack_int( data, p, m_localTime ) ) return false;
...
bool CRoomPresencePacket::unpack( QByteArray &from )
{
	...
	switch ( ( QXmppPresence::Type )t ) {
		...
		case QXmppPresence::Probe: break;
