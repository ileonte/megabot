...
#define RETURN_FALSE                                                                                                           \
	{                                                                                                                        \
		if (LUALIB->lua_gettop(L) > t)                                                                                     \
			LUALIB->lua_pop(L, LUALIB->lua_gettop(L) - t);                                                               \
		return false;                                                                                                      \
	}

	LUALIB->lua_newtable(L);
	while (i < size) {
		if (!__parser_type_and_length(start + i, &o, &l, &c))
			RETURN_FALSE;
		i += c;
		if (i >= size)
			RETURN_FALSE;
		...
		switch (o) {
			...
			case 'b': {
				if (!__internal_unserialize_bool(L, l, start + i))
					RETURN_FALSE;
				break;
			}
			case 'i':
			case 'f': {
				if (!__internal_unserialize_number(L, l, start + i))
					RETURN_FALSE;
				break;
			}
			case 's': {
				if (!__internal_unserialize_string(L, l, start + i))
					RETURN_FALSE;
				break;
			}
			case 'a': {
				if (!__internal_unserialize_table(L, l, start + i))
					RETURN_FALSE;
				break;
			}
			default: {
		...
		switch (o) {
			case 'n': {
				if (!__internal_unserialize_nil(L, l, start + i))
					RETURN_FALSE;
				break;
			}
			case 'b': {
...
#define FAIL                                                                                                                   \
	...
	if (!__parser_type_and_length(start, &o, &l, &c))
		FAIL;
	if (c + l >= size)
		FAIL;
	if (start[c + l] != ';')
	...
	switch (o) {
		case 'n': {
			if (!__internal_unserialize_nil(L, l, start + c))
				FAIL;
			break;
		}
		case 'b': {
			if (!__internal_unserialize_bool(L, l, start + c))
				FAIL;
			break;
		}
		case 'i':
		case 'f': {
			if (!__internal_unserialize_number(L, l, start + c))
				FAIL;
			break;
		}
		case 's': {
			if (!__internal_unserialize_string(L, l, start + c))
				FAIL;
			break;
		}
		case 'a': {
			if (!__internal_unserialize_table(L, l, start + c))
				FAIL;
			break;
		}
		default: {
...
#define PRESENCE(__i)                                                                                                          \
	m_luaLib->lua_pushinteger(L, (int)QXmppPresence::__i);                                                                   \
	m_luaLib->lua_setglobal(L, "PRES_" #__i)
	PRESENCE(Error);
	PRESENCE(Available);
...
#define STATUS(__i)                                                                                                            \
	m_luaLib->lua_pushinteger(L, (int)QXmppPresence::__i);                                                                   \
	m_luaLib->lua_setglobal(L, "STAT_" #__i)
	STATUS(Online);
	STATUS(Away);
	STATUS(XA);
	STATUS(DND);
